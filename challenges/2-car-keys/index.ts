...
// Takes an array of binary strings and returns the string represented by the binary strings.
function getName(binaries: string[]): string {
  return binaries
    .map((binary) => parseInt(binary, 2)) // convert to ascii codes
    .map((charCode) => String.fromCharCode(charCode)) // convert to letters
    .join('');
}
